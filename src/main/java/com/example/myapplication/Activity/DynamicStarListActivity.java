...
public class DynamicStarListActivity extends BaseActivity{
    ...
    public static DynamicStarListActivity DynamicListActivity_instance;
        ...
        mDatabase.child("user").child(userId).child("likeDynamic").get().addOnCompleteListener(new OnCompleteListener<DataSnapshot>() {
            ...
            public void onComplete(@NonNull Task<DataSnapshot> task) {
                if (!task.isSuccessful()) {
                    Toast.makeText(DynamicStarListActivity.this, "网络故障，请稍后重试", Toast.LENGTH_SHORT).show();
                } else {
    ...
    private void initComponent() {
        ...
        adapter = new DynamicRecyclerAdapter(getApplicationContext(),DynamicStarListActivity.this);
    ...
    private void initLoadDynamic(){
        showProgressDialog(DynamicStarListActivity.this,"加载动态中……");
        loadDynamic();
    }
            ...
            public void onComplete(@NonNull Task<DataSnapshot> task){
                if(task.isSuccessful()){
                    ...
                    if(position == myDynamic.size()){
                        Toast.makeText(DynamicStarListActivity.this,"已到最底部",Toast.LENGTH_SHORT).show();
                    }else{
                    ...
                    Toast.makeText(DynamicStarListActivity.this,"加载失败",Toast.LENGTH_SHORT).show();
                }
            }
        });
    }
